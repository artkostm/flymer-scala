...
private object ActionIfInvalid {
  def apply(msg: String, valid: Boolean): TextInputEditText => Option[Unit] =
    if (valid) _ => Some(())
    else editText => {
  ...
  def render(dispatch: Dispatcher)(implicit ctx: ContextWrapper) : Ui[View] = l[ScrollView] (
    l[LinearLayout] (
      ...
      w[AppCompatButton]
        ...
        <~ On.click(Ui {
        if (validate) dispatch(FlymerLogin(emailSlot.map(textExtractor), passwordSlot.map(textExtractor)))
      }),
      w[TextView]
        <~ text("Login via Vk")
        <~ TextTweaks.size(16)
